      ...
      try {
        ...
        setStats({
          activeListings: allPets.filter((p) => p.status === "Available")
            .length,
          pendingRequests: allPets.filter(
            (p) => p.status === "Reserved" || p.status === "OnHold",
          ).length,
    ...
    <div className="flex min-h-screen bg-[#f5f7fa]">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <div className="max-w-5xl">
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-xl font-semibold text-[#1B3A4B] mb-0.5">
          ...
          {!loading && totalPets > 0 && (
            <div className="flex items-center justify-between">
              <p className="text-xs text-gray-400">
                Showing {pets.length} of {totalPets} listings
              </p>
              <div className="flex items-center gap-2">
                <button
