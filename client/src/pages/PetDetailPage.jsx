...
function ChatPopup({ pet, onClose }) {
  ...
  const sendMessage = async () => {
    ...
    setInput("");
    socketRef.current?.emit("send_message", { conversation_id: conversationId, content });
    setMessages((prev) => [...prev, {
      id: Date.now(), conversation_id: conversationId,
      sender_id: currentUser.id, content, file_url: null,
      createdAt: new Date().toISOString(),
      sender: { id: currentUser.id, first_name: currentUser.first_name || "You", role: currentUser.role },
    }]);
    setSending(false);
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      socketRef.current?.emit("send_message", {
        ...
        file_url: res.data.file_url, file_type: res.data.file_type,
      });
    } catch (err) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden" style={{ height: "460px" }}>
      <div className="flex items-center justify-between px-4 py-3 bg-blue-600 text-white">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-blue-100">{pet.shelter?.name || "Shelter"}</p>
          </div>
        </div>
      ...
      <div className="bg-amber-50 border-b border-amber-100 px-3 py-2">
        <p className="text-xs text-amber-700">🔒 You're chatting anonymously. Your identity is revealed only after applying.</p>
      </div>
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
        {loading && <div className="flex items-center justify-center h-full"><Loader2 size={20} className="animate-spin text-gray-300" /></div>}
        {!loading && messages.length === 0 && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center"><div className="text-3xl mb-2">🐾</div><p className="text-xs text-gray-400">Say hello to ask about {pet.name}!</p></div>
          </div>
        )}
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${isMe(msg) ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[70%] rounded-2xl px-3 py-2 ${isMe(msg) ? "bg-blue-600 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"}`}>
              {msg.file_url
                ? <img src={msg.file_url} alt="attachment" className="rounded-lg max-w-full max-h-40 object-cover" />
      ...
      <div className="border-t border-gray-100 px-3 py-2 flex items-center gap-2">
        <button onClick={() => fileInputRef.current?.click()} disabled={uploadingImage} className="text-gray-400 hover:text-blue-500 transition-colors shrink-0">
...
const PetDetailPage = () => {
  ...
  const handleAdopt = () => {
    if (!UserService.isAuthenticated()) { navigate("/login", { state: { from: `/pets/${id}` } }); return; }
  ...
  const handleChat = () => {
    ...
    if (currentUser?.role === "shelter" || currentUser?.role === "admin") {
      alert("Shelter accounts cannot send adoption inquiries."); return;
    }
    setChatOpen(true);
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate("/browse")} className="text-sm text-blue-600 hover:underline">← Back to Browse</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <button onClick={() => navigate("/browse")} className="text-sm text-gray-500 hover:text-blue-600 mb-6 flex items-center gap-1">
          ← Back to Browse
        </button>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden p-8">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="w-full md:w-80 shrink-0">
              ...
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-3">Shelter & Adoption</h3>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-xs font-bold text-gray-600">
                    {pet.shelter?.name?.charAt(0) || "S"}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-800">{pet.shelter?.name || "Unknown Shelter"}</p>
                    <p className="text-xs text-gray-400">{pet.shelter?.city}, {pet.shelter?.state}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Adoption Fee</p>
                  <p className="text-2xl font-bold text-gray-900">{pet.adoption_fee ? `₹${pet.adoption_fee}` : "Free"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-500 text-sm mb-6">{pet.breed} • {pet.age} {pet.age === 1 ? "Year" : "Years"} Old</p>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    { label: "Vaccinated", value: pet.vaccinated ? "Yes, Up-to-date" : "No" },
                    { label: "Sterilized", value: pet.sterilized?.replace("_", " ") },
                    { label: "Health Record", value: pet.health_status },
                    { label: "Good with Kids", value: pet.good_with_kids === true ? "Yes" : pet.good_with_kids === false ? "No" : "—" },
              ...
              {pet.rescue_story && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-gray-700 mb-3">Rescue Story</h3>
                  <div className="bg-blue-50 rounded-xl p-4">
                    <p className="text-sm text-gray-700 leading-relaxed italic">"{pet.rescue_story}"</p>
                  </div>
                </div>
              )}

              <div className="flex gap-3">
                <button onClick={handleAdopt} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 transition-colors text-sm">
                  Apply for Adoption →
                </button>
                <button onClick={handleChat} className="flex items-center gap-2 px-4 py-3 border-2 border-blue-600 text-blue-600 rounded-xl font-medium hover:bg-blue-50 transition-colors text-sm">
      ...
      {chatOpen && <ChatPopup pet={pet} onClose={() => setChatOpen(false)} />}
    </div>
  );
};
